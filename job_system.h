...
namespace engine {
    ...
    namespace core {
        ...
        private:
            ...
            u64 m_JobsTodo; // tracks state of execution of main thread
            std::atomic<u64> m_JobsDone; // tracks state of execution around worker threads
        };
        ...
        JobSystem<render_jobs, audio_jobs, network_jobs, thread_pool_jobs>::JobSystem() {
            ...
            ASSERT(threadPoolSize > 0, "JobSystem(): invalid condition threadPoolSize <= 0")
            renderScheduler = createScope<JobScheduler<render_jobs>>(1, ThreadFormat(ThreadPriority::HIGHEST,
                                                                                     "RenderThread"));
            audioScheduler = createScope<JobScheduler<audio_jobs>>(1, ThreadFormat(ThreadPriority::HIGHEST,
                                                                                   "AudioThread"));
            networkScheduler = createScope<JobScheduler<network_jobs>>(1, ThreadFormat(ThreadPriority::HIGHEST,
                                                                                       "NetworkThread"));
            threadPoolScheduler = createScope<JobScheduler<thread_pool_jobs>>(threadPoolSize,
                                                                              ThreadFormat(ThreadPriority::NORMAL,
                                                                                           "ThreadPoolWorker"));
        }
        ...
        template<size_t jobs_capacity>
        void
        JobScheduler<jobs_capacity>::execute(u32 jobsPerThread, u32 jobSize, const std::function<void(JobArgs)> &job) {
